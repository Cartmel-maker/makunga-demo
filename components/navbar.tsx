  ...
  return (
    <header className="relative z-20 flex items-center justify-between border-b border-white/10 pb-5">
      ...
      <div className="flex items-center gap-3">
        <a href="#support" className="hidden items-center gap-2 rounded-lg bg-primary px-4 py-2.5 font-sans text-sm font-bold text-primary-foreground sm:inline-flex">Contact Desk <ArrowUpRight className="size-4" /></a>
        <button type="button" onClick={() => setOpen(!open)} className="rounded-lg border border-white/15 bg-zinc-900/80 p-2.5 text-white lg:hidden" aria-expanded={open} aria-label="Toggle navigation menu">{open ? <X className="size-5" /> : <Menu className="size-5" />}</button>
      </div>
      {open ? <nav className="absolute right-0 top-14 flex w-64 flex-col gap-1 rounded-xl border border-white/10 bg-zinc-950/95 p-3 shadow-2xl backdrop-blur-xl lg:hidden" aria-label="Mobile navigation">{links.map(([label, href]) => <a key={href} href={href} onClick={() => setOpen(false)} className="rounded-lg px-3 py-2.5 font-sans text-sm text-zinc-300 hover:bg-zinc-800 hover:text-white">{label}</a>)}<a href="#support" onClick={() => setOpen(false)} className="mt-2 inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 font-sans text-sm font-bold text-primary-foreground">Contact Desk <ArrowUpRight className="size-4" /></a></nav> : null}
    </header>
  )
}
